...
def ia_plays(user_game, iaDTO, game, gameDTO):
    userGame.move()
    if game.game_over():
...
def start_game(game_id, user):
    ...
    userGames = gmodels.UserGame.objects.filter(game__id=game.id)
    ...
    if userGame2.ia and userGame2.userNumber == gameDTO.turn:
        (moveX, moveY) = play(userGame1.posUserX, userGame1.posUserY, userGame2.posUserX, userGame2.posUserY,
                              gameDTO.gameState, userGame2,
                              gameDTO.possible_actions(userGame2.posUserX, userGame2.posUserY, userGame2.userNumber),
                              gameDTO.turn)
...
def apply_move(game_id, user, movement):
    userGame = gmodels.UserGame.objects.filter(game__id=game_id, userId__id=user.id).first()
    if not userGame:
    ...
    game = userGame.game
    userGame2 = gmodels.UserGame.objects.filter(game__id=game.id).exclude(userId__id=user.id).first()
    ...
    if not gameDTO.winner:
        ...
        if userGame2.ia and not gameDTO.game_over():
            (moveX, moveY) = play(newPosX, newPosY, userGame2.posUserX, userGame2.posUserY, gameDTO.gameState,
                                  userGame2, gameDTO.possible_actions(userGame2.posUserX, userGame2.posUserY,
                                                                      userGame2.userNumber), gameDTO.turn)
...
def resume_game(game_id, user_id):
    ...
    gameDTO = mapGame(game)
    userGames = gmodels.UserGame.objects.filter(game__id=game_id).all()
...
def create_game(form, user_id):
    ...
    gmodels.UserGame.objects.create(userId=user, game=game, color=hex_color, userNumber=1)
...
def verrification_before_join(label, form, game_id):
    ...
    if hex_color == 0:
        raise ForbidenColorError()
    if gmodels.UserGame.objects.filter(game__id=game_id, color=hex_color).exists():
...
def join_ia(game_id, ia_id, form):
    ...
    gmodels.UserGame.objects.create(ia=ai, game=game, color=hex_color, userNumber=2)
...
def join_game(game_id, user, form):
    ...
    gmodels.UserGame.objects.create(userId=user, game=game, color=hex_color, userNumber=2)
...
def train(ia_id, form):
    ...
    for __ in range(nb_games):
        ...
        userGame1 = gmodels.UserGame.objects.create(game=game, ia=ia, color=0, userNumber=1)
        userGame2 = gmodels.UserGame.objects.create(game=game, ia=ia, color=1, userNumber=2)
        ...
        while not gameDTO.game_over():
            ...
            (moveX, moveY) = play(userGame1.posUserX, userGame1.posUserY, userGame2.posUserX, userGame2.posUserY,
                                  gameDTO.gameState, current_userGame,
                                  gameDTO.possible_actions(current_userGame.posUserX, current_userGame.posUserY,
                                                           current_userGame.userNumber), gameDTO.turn)
